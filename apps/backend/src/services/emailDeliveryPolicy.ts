...
export function assertNewsletterDeliveryAllowed(recipients: Array<{ parentEmail?: string | null }>): void {
  ...
  if ((env.NODE_ENV !== 'production' || env.NEWSLETTER_DEMO_MODE === 'true') && allowed.size === 0) {
    throw new DeliveryPolicyError('Local or demo newsletter delivery requires NEWSLETTER_TEST_RECIPIENTS')
  }
