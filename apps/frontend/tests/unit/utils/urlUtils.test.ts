...
describe('urlUtils redirect helpers', () => {
  ...
  it('only accepts safe in-app redirect targets', () => {
    ...
    expect(isSafeAppRedirectPath('/admin')).toBe(true)
    expect(isSafeAppRedirectPath('/admin/newsletters/preview')).toBe(true)
    expect(isSafeAppRedirectPath('/admin/../../login')).toBe(false)
    expect(isSafeAppRedirectPath('/\\evil.example/path')).toBe(false)
    expect(isSafeAppRedirectPath('/administer')).toBe(false)
  })
})
