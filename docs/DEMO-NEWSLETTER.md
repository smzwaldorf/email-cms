...
4. For these controlled demo inboxes only, explicitly confirm initial newsletter subscription with `DEMO_SUBSCRIPTIONS_CONFIRMED=true` in **CMS before first apply**. Otherwise both preferences start pending and cannot send. A seed rerun never upgrades consent, restores disabled relationships, or clears a suppression; existing pending/opt-out preferences require the separate audited consent-management workflow (#31).
...
Resetting is a separate operator action; `seed:demo` never resets either database. After restoring the current schemas/migrations and normal Auth administrator/OIDC configuration, run the Auth seed first, then CMS using the commands above. The saved template and A/B class relationships are included. Delivery remains disabled until configured, and newsletter preferences remain pending unless `DEMO_SUBSCRIPTIONS_CONFIRMED=true` is explicitly set for the first CMS apply. Do not run the older school fixture first: it can claim these email addresses under different identities.
