...
The system SHALL create a preparation job that composes send-ready recipient payloads using pinned references to newsletter content revision, template revision, and recipient membership snapshot.
...
The system SHALL validate each prepared payload for required sections, supported token usage, and mandatory resolved values before marking it ready for delivery handoff.
