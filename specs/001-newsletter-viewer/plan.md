...
### shadcn/ui 正當性（「簡潔和務實」原則對齐）

**決策**: 採用 shadcn/ui (Radix UI + Tailwind CSS 集成) 而非自建元件庫

**正當理由**:
- **複雜度減少**: shadcn/ui 提供預構建的可訪問性元件（Button、Dialog、Popover），無需自行實現 ARIA 標籤、鍵盤導航、焦點管理
- **避免重複工作**: 不必從零開始寫按鈕、導航欄、加載指示器等基礎 UI 元件
- **可測試性**: Radix UI 元件經過充分測試，降低 UI 層的測試負擔
- **維護成本**: 依賴於活躍社區（Radix UI），更新和安全補丁有保障
- **無額外膨脹**: shadcn/ui 是基於複製代碼而非 npm 包，允許定制和移除未使用的代碼，符合「務實」原則

**權衡**: 單一新依賴 (Radix UI)，但換取數十個自建元件的複雜性。淨收益：簡潔度提升。
