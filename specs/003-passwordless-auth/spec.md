...
- Q: 使用者在一個裝置上登出時，是否應該在所有裝置上登出，還是僅在該裝置上登出？ → A: 僅在該裝置上登出；其他裝置的工作階段保持活躍。
- Q: 當使用者使用電子郵件進行身份驗證（Google OAuth 或魔法連結），而該電子郵件已經存在於系統中但使用不同的驗證方法時，應該發生什麼？ → A: 自動合併到現有帳戶（視為帳戶復原/新增另一種身份驗證方法）。
- Q: 當使用者試圖將他們的電子郵件變更為系統中已被另一個帳戶使用的電子郵件時，應該發生什麼？ → A: 阻止變更並顯示「該電子郵件已被另一個帳戶使用」的錯誤訊息。
...
5. **已知** 新電子郵件地址使用魔法連結，**當** 他們成功驗證，**則** 自動建立新使用者帳號；如果該電子郵件已存在於系統中，則登入現有帳戶而不是建立重複帳戶
...
3. **已知** 使用者從裝置 A 登出，**當** 他們檢查裝置 B，**則** 裝置 B 保持登入狀態（登出僅限於該裝置）
...
管理員可以在稽核日誌中查看認證事件（登入嘗試、OAuth 完成、魔法連結使用）以進行合規性和安全監視。速率限制可防止對魔法連結的暴力攻擊。可疑活動會觸發通知。管理員還可以在使用者管理儀表板中查看任何使用者的活躍工作階段，並強制無效化該使用者的所有登入工作階段（例如，在安全事件或帳號洩露時）。

**為什麼是此優先級**: 安全日誌和速率限制對於處理敏感使用者資料的生產系統至關重要。合規性和威脅偵測取決於稽核跟蹤。管理員強制登出功能對於事件回應和帳號洩露恢復至關重要。

**獨立測試**: 可以通過執行認證操作並驗證他們出現在具有適當時間戳和詳細資料的稽核日誌中來測試。可以通過模擬重複失敗嘗試來測試速率限制。可以通過作為管理員選擇一個使用者並強制登出其所有工作階段，然後驗證其他裝置被登出來獨立測試管理員工作階段管理。
...
5. **已知** 管理員在使用者管理儀表板中，**當** 他們選擇一個使用者並點擊「強制登出所有工作階段」，**則** 該使用者的所有活躍登入工作階段被無效化，所有裝置在下次操作時要求重新驗證
...
1. **已知** 已登入使用者在帳號設定中，**當** 他們輸入新電子郵件地址並點擊「變更電子郵件」，**則** 系統驗證該電子郵件在系統中不存在；如果存在則顯示錯誤，否則驗證連結發送到新地址並顯示確認訊息
...
- **FR-010**: 系統必須在使用者登出時清除該裝置上的所有權杖（存取和重新整理），並在伺服器端無效化該特定工作階段的重新整理權杖；其他裝置上的工作階段保持有效
...
- **FR-027a**: 系統必須記錄所有管理員行動到稽核日誌，包括強制登出事件（包括目標使用者、管理員 ID、時間戳和原因）
...
- **FR-031a**: 系統必須為管理員提供儀表板功能，允許他們查看任何使用者的所有活躍工作階段，並且能夠強制無效化該使用者的所有登入工作階段（跨所有裝置）

**帳號連結和衝突解決**:
- **FR-041**: 系統必須將電子郵件視為唯一識別符；當使用者使用已存在於系統中的電子郵件進行身份驗證時（透過不同的方法），系統必須自動將新認證方法連結到現有帳戶而不是建立新帳戶

**電子郵件變更和工作階段無效化**:
- **FR-032**: 系統必須提供帳號設定介面，允許已驗證使用者輸入新電子郵件地址
- **FR-032a**: 系統必須驗證新電子郵件地址的唯一性；如果新電子郵件已被系統中的另一個帳戶使用，則拒絕變更請求並顯示清楚的錯誤訊息
