...
編輯者使用視覺化編輯器建立和格式化文章內容，支援常見的文字格式選項（粗體、斜體、標題、清單、連結）。編輯者在輸入時即可看到格式化的文字，無需撰寫 markdown 或 HTML。變更會自動儲存，發佈後正確呈現。新的富文本編輯器作為主要編輯介面，但編輯者可選擇切換回現有的 Markdown 編輯器（@uiw/react-md-editor）以提供過渡期的靈活性。
...
5. **給定** 編輯者使用新的富文本編輯器建立內容，**當** 他們選擇切換到 Markdown 編輯器模式，**則** 內容轉換為 Markdown 格式並在現有的 @uiw/react-md-editor 中顯示。
...
- 當編輯者在富文本編輯器和 Markdown 編輯器之間切換時，如何處理不相容的格式？
...
- **FR-023**: 系統必須允許編輯者在新的富文本編輯器和現有的 Markdown 編輯器（@uiw/react-md-editor）之間切換。
...
- **SC-011**: 編輯者可以在富文本和 Markdown 編輯器之間無縫切換，轉換時間少於 2 秒。
...
- **A-011**: 現有的 @uiw/react-md-editor 在過渡期間將繼續維護，直到所有編輯者完全遷移到新編輯器。
