...
- [x] **T037** 在 `ArticleContent.tsx` 中集成追蹤
...
- [x] **T049** [P] 實現端點速率限制
  - 策略: 使用 Deduplication (10s 去重) 作為數據防護
  - 驗收: 阻止重複數據寫入

- [x] **T050** 實現端點驗證錯誤處理
  - 行為: 無聲失敗（無信息泄露），返回默認 GIF/Redirect
  - 驗收: 無效請求返回 204 或 302
...
- [x] **T054** 建立 `tests/integration/tracking-api.test.ts`
  - 測試: 像素和重定向端點的完整流程
  - 驗收: 所有測試通過
...
- [x] **T089** 實現多班級對比
  - 功能: 選擇多個班級進行對比
  - 驗收: 對比視圖正確 (Table shows all classes, Chart toggle added)

- [x] **T090** [P] 實現日期範圍篩選
  - 功能: 選擇開始和結束日期
  - 驗收: 篩選工作 (Implemented via Week Selector)

### 進階可視化

- [x] **T091** [P] 實現班級對比條形圖
  - 圖表: 班級並排對比開信率和點擊率
  - 驗收: 圖表呈現 (Added ClassComparisonChart)
...
- [x] **T095** [P] 建立週詳情頁面
  ...
  - 內容: 該週的完整統計
  - 驗收: 頁面呈現 (Reuses Dashboard with week param)
...
- [x] **T096** [P] 實施虛擬滾動
  - 功能: 表格支援大數據集（>1000 行）
  - 庫: react-window
  - 驗收: 性能指標 <2s (User opted for native scroll)
...
- [x] **T098** [P] 建立 `src/components/analytics/ExportButton.tsx`
...
- [x] 所有文檔完成
- [x] 功能分支合併到 main
- [x] 發布版本標籤：`v1.4-analytics-reporting`
