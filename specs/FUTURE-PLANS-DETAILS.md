> **Ownership revision (2026-09-05):** [Current contract](docs/SMZ_AUTH_CMS_CONTRACT.md). SMZ Auth owns login, roles, school relationships and per-app admission; CMS consumes live facts for action policy. Earlier local role/bulk-assignment and magic-link plans are superseded. Historical completion statements are preserved as history, not current verification.
...
**現況**: 已具備閱讀器、認證、富文本編輯、媒體治理、管理後台核心流程與分析儀表板
**目標**: 補完郵件整合、個人化發送、模板與進階權限，形成完整 CMS
...
**優先順序調整（2026-03-19）**：
- 已完成：**分析與報表**、**媒體管理**、**富文本編輯器核心**
- 目前優先順序 1️⃣：**管理後台系統剩餘項目** - 內容生命週期與權限 UI
- 目前優先順序 2️⃣：**郵件整合 + 個人化** - 核心價值主張
- 目前優先順序 3️⃣：**模板管理**
- 目前優先順序 4️⃣：**進階權限管理**

> **註**：驗證與權限系統、富文本編輯器核心、分析與媒體治理已落地；以下排序聚焦剩餘能力。
...
**功能範圍（已落地）**：
- **開信率追蹤**：已具備 tracking pixel 與事件聚合
- **點擊率追蹤**：已具備 redirect / click tracking
- **視覺化儀表板**：已實作分析頁面、表格與圖表元件
- **報表匯出**：已支援 CSV/Excel 匯出流程
- **分班統計**：已具備班級維度分析與對比
...
**功能範圍（已落地）**：
- **媒體庫系統** - 已可集中管理上傳的圖片、音訊、影片
  - 按日期、類型、標籤組織媒體檔案
  - 搜尋與過濾功能（按檔案名稱、上傳日期、尺寸）
  - 媒體預覽（縮圖、音訊播放器、影片預覽）
  - 管理端安全刪除流程

- **媒體重用與共享** - 已可在多篇文章間重用媒體
  - 參考計數追蹤（某媒體被使用在幾篇文章）
  - 防止刪除正在使用的媒體
  - 媒體使用歷史（查看某媒體在哪些文章中使用）
  - 編輯器插入現有媒體與複製文章保留引用

- **媒體優化與變體** - 已具備自動生成與狀態追蹤
  - 自動建立縮圖版本（編輯器瀏覽、郵件預覽）
  - 自動產生 WebP 格式（現代瀏覽器最佳化）
  - 變體 ready/failed 狀態
  - 音訊最佳化與格式就緒資訊

- **媒體使用儀表板** - 已可視化媒體管理與使用情況
  - 儲存空間使用統計
  - 媒體類型分佈圖
  - 最常用媒體排名
  - 未使用媒體清單（可安全刪除）
...
**優先順序 3️⃣：模板管理**
```bash
/speckit.specify 模板管理
```
- 預估：2-3 週
- 關鍵功能：週報模板、複製工作流、固定內容區域

**優先順序 4️⃣：進階權限管理**
```bash
/speckit.specify 進階權限管理
```
- 預估：2-3 週
- 關鍵功能：細粒度角色控制、多角色衝突處理、存取審核
