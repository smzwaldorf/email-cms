...
- 🔄 **階段 2：富文本編輯器 + 內容管理增強（進行中 - 004-rich-text-editor）**
  - 已完成：TipTap 編輯器基礎、YouTube 節點、音訊節點、圖片上傳、媒體庫基礎
  - 進行中：媒體重用、優化管道
  - 分支：`004-rich-text-editor`
- 🔄 **階段 3：管理後台系統（並行開發 - 005-admin-dashboard）**
  - 已完成：管理儀表板基礎、週報管理、班級管理、用戶管理、審計日誌、批量導入
  - 進行中：內容管理介面、權限控制 UI 完善
