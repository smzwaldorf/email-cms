...
Do not provision another paid cluster or point CMS at Auth's `smz-auth` logical database/Hyperdrive. The live PlanetScale SQL console confirmed `current_database() = smz-cms` and zero public tables on 2026-09-12. A CMS-only role and uncached Hyperdrive `686ed1534b77435eb5537fabba62e61c` now target this database. The credential is stored in GitHub production as `CMS_DATABASE_URL`, and the replacement credential has been saved after the user completed the PlanetScale reset.
...
2. Obtain the direct `smz-cms` database connection using verified TLS. Set `CMS_DATABASE_URL` securely; do not commit it. On the verified empty database only, run `npm run cloudflare:schema`. The script checks the actual database name, refuses existing public tables and applies schema in a transaction. It is intentionally not a recurring migration runner. Do not use the destructive local reset scripts in production.
...
Production **secrets**: `CLOUDFLARE_API_TOKEN` and direct verified-TLS `CMS_DATABASE_URL`. No provider sending secrets are required while delivery is disabled. Database credentials and API tokens must never be Vite variables.
