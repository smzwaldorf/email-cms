        ...
        <div className="mb-8">
          <div className="flex items-center justify-between bg-white/70 backdrop-blur-sm shadow-sm rounded-2xl px-6 py-4 border border-waldorf-cream-200">
            {latestNewsletter ? (() => {
              ...
              const routeKey = latestNewsletter.week_number || latestNewsletter.id
              const isWeekNumber = latestNewsletter.week_number && /^\d{4}-W\d{2}$/.test(String(latestNewsletter.week_number))
