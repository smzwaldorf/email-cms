...
import {
  ...
  trackMetricOutcome,
} from '../../../src/services/emailPlatform/utils.ts'
import { verifyKitWebhookSecret } from '../../../src/services/emailPlatform/webhook.ts'
import { EmailPlatformConfig, EmailPlatformError } from '../../../src/types/emailPlatform.ts'
...
function createKitAdapter(config: EmailPlatformConfig): KitAdapter {
  return new KitAdapter(config)
}
...
function getRetryConfig(jobOrEvent: { max_attempts?: number | null }, config: EmailPlatformConfig) {
  return {
...
async function resolveFamilyByEmail(adminClient: AdminClient, emailAddress: string | null) {
...
export async function loadRecipientRecord(adminClient: AdminClient, familyId: string) {
...
async function getMappingForFamily(adminClient: AdminClient, familyId: string) {
  const { data, error } = await adminClient
...
async function markJobProcessing(adminClient: AdminClient, jobId: string) {
...
async function markWebhookProcessing(adminClient: AdminClient, eventId: string) {
...
export async function processSyncJob(
  ...
  const adapter = createKitAdapter(config)
  ...
  try {
    ...
    const existingMapping = await getMappingForFamily(adminClient, recipient.familyId)
    const syncOutcome = await adapter.upsertSubscriber({
      payload,
      previousExternalSubscriberId: existingMapping?.external_subscriber_id,
    })
...
export async function processPendingSyncJobs(
  ...
  for (const job of jobs) {
    const status = await processSyncJob(adminClient, job, config)
